...
const mapStatusForDisplay = (status: string | undefined): LogisticsStatus => {
  if (status === 'REJECT') {
    return 'ASSIGNED';
  }
  
  const validStatuses: LogisticsStatus[] = ['ASSIGNED', 'PENDING', 'IN_PROGRESS', 'COMPLETED', 'REJECT'];
  
  if (status && validStatuses.includes(status as LogisticsStatus)) {
    return status as LogisticsStatus;
  }
...
                <StatusStepBar
                  currentStatus={mapStatusForDisplay(taskDetail.logisticsStatus as LogisticsStatus)}
                  type="outbound"
                />
              </FormGroup>
